...
class GeotelaSupabaseUser extends BaseAuthUser {
  GeotelaSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> geotelaSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GeotelaSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
